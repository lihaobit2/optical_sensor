var s_tc_GlobalContainer=new TagContainer('GlobalContainer');
...
function s_doPlugins(s) {
	...
	if(s.prop12){
        ...
        s.eVar12=s.prop12;
        s.eVar12=s.getValOnce(s.eVar12,'ev12',0);
        if(s.eVar12){
	...
	if(!s.eVar16) s.eVar16=s.getQueryParam('intcmp')
	if(s.eVar16){
		s.eVar16=s.getValOnce(s.eVar16,'ev16',0)
	...
	s.eVar13=s.prop13=s.TimeParting('h','-5'); // Set hour 
	s.eVar14=s.prop14=s.TimeParting('d','-5'); // Set day 
	s.eVar15=s.prop15=s.TimeParting('w','-5'); // Set weekday v.Weekend
	
	/*New vs Repeat*/
	s.prop17 = s.getNewRepeat(365);
	
	/* Copy props to eVars */
	if(s.pageName&&!s.eVar11) s.eVar11=s.pageName;
	if(s.channel&&!s.eVar10) s.eVar10=s.channel;
	...
	s.eVar21=s.getQueryParam('et_cid'); 
	s.eVar21=s.getValOnce(s.eVar21,'ev21',0);
	s.eVar22=s.getQueryParam('et_rid'); 
...
s.visitorNamespace = "abm"
s.trackingServer = "metrics.theb2bt.com"
s.trackingServerSecure = "smetrics.theb2bt.com"
...
s.setTagContainer("GlobalContainer")
