...
uint16_t NPI_WriteString( char *buf);


/* Declare some strings */
const char     welcomeString[]  = "abcd\r\n";//"EFM32 RS-232 - Please press a key\n";
const char     overflowString[] = "\n---RX OVERFLOW---\n";
const uint32_t welLen           = sizeof(welcomeString) - 1;
const uint32_t ofsLen           = sizeof(overflowString) - 1;
char astr[32];
#define MAX_PRINT_STR_LEN (sizeof(astr) - 1)

/* Declare a circular buffer structure to use for Rx and Tx queues */
#define BUFFERSIZE          128//256
...
static void gpioSetup(void)
{
  ...
  /* Configure PC8 as input and enable interrupt. (PB0) */
  ...
  /* Configure PC9 as input and enable interrupt. (PB1) */
...
void uartSetup(void)
{
  ...
  CMU_Calibrate(1000, cmuOsc_LFXO);
...
uint16_t NPI_WriteString(char *buf)
{
    ...
    if((len == 0) || (len > MAX_PRINT_STR_LEN))
    {
    	return 1;
    }
     
	uartPutData((uint8_t*) buf, len);
